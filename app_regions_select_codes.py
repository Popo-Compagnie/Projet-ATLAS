...
with st.form("form"):
    ...
    # --- Listes & mapping pour menus déroulants
    CATEGORIES = [
        "Électronique", "Maison", "Mode", "Alimentaire",
        "Santé", "Jouets", "Livres", "Autre"
    ]
    CONTAINERS = [
        "Carton", "Palette", "Boîte", "Sac", "Caisse", "Autre"
    ]
    REGIONS = {
    ...
    category = st.selectbox("Catégorie produit", CATEGORIES)
    container = st.selectbox("Type de contenant", CONTAINERS)
    ...
    # Valeurs envoyées au modèle (codes numériques attendus par le pipeline)
    seller_region = REGION_CODE_FROM_NAME[seller_region_label]
    customer_region = REGION_CODE_FROM_NAME[customer_region_label]
